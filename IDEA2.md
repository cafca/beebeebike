...
- **Routing engine, phase 1:** GraphHopper running on a development machine (containerized), accessed by the apps over HTTP. It is a development dependency, not an on-device component. JVM-based servers cannot run inside an iOS app.
- **Routing engine, phase 2+:** A custom engine built later, embedded directly in the app. This is a hard prerequisite for shipping to end users and for true offline operation on iOS — not merely a performance or experimentation upgrade. Phase 1 hides the engine behind an interface so the swap is mechanical.
...
- **Offline-first:** Required mid-term and a non-negotiable shipping requirement. Phase 1 is network-dependent and not shippable; the apps run as thin clients against a developer-hosted GraphHopper instance. True offline operation arrives with the phase 2 on-device custom engine.
...
### GraphHopper Service (phase 1, containerized, off-device)

Purpose: Answer routing requests over HTTP for Berlin bicycle routes. Acts as a stand-in for the future custom engine during development.

Runs on a developer machine or the Hetzner VPS, never on the user's device. iOS does not allow embedding a JVM or running container runtimes inside an app, and even if it did, the Berlin graph and JVM footprint would be unsuitable. The macOS app talks to it over the network like any other client; "localhost" works for solo development but is not a deployment model.

Owns: the routing graph it builds from OSM data, the HTTP endpoint.

Depends on: OSM data (or a pre-built graph file), Docker runtime, network reachability from the apps.

Does not: serve tiles, store user data, run on end-user devices, or persist beyond development. It will be retired entirely when the custom engine arrives.
...
This is the work that turns the project from a development prototype into something shippable. As long as routing depends on GraphHopper, the apps cannot run without a developer-hosted server, which means they cannot be distributed. The custom engine is what makes iOS deployment possible at all; treating it as merely an experimentation upgrade understates its importance.

Phase 1 must get right: the routing engine interface must be designed for the richer contract the custom engine will provide, not trimmed to what GraphHopper returns easily. Specifically: per-segment metadata (surface, infrastructure type, way ID) must flow through the interface even if GraphHopper's support is partial. If the interface is narrowed to GraphHopper's output, the custom engine will be forced to fit into that mold. The interface must also tolerate a synchronous, in-process implementation — not assume a network round-trip — so that the eventual on-device engine fits without contortion.
...
Phase 1 is fundamentally online: the apps are thin clients against a developer-hosted GraphHopper instance and a Hetzner-hosted tile server. Neither can be made offline within phase 1 — GraphHopper cannot run on the device, period. Offline operation is a phase 2 deliverable that depends on the custom engine being embedded in the shared core.

Phase 1 must get right: the routing engine interface must return a structured "unavailable" error rather than crashing or hanging when the server is unreachable, so that the same error-handling code path can be reused once the only failure mode is "graph not loaded yet." The tile source interface must be structured so a local cache or bundled-tile implementation can be substituted without changing anything above it. MapLibre's built-in tile cache provides a partial offline experience for recently viewed areas at no extra cost and should be sized generously from the start.

Can defer: explicit area download UI, background tile refresh, multi-region tile management, any code that actually runs against an embedded engine.
...
Graph preparation runs on the developer machine or VPS that hosts the container, the first time the container starts against fresh OSM data, or against a pre-built graph file if one is provided. It never runs on a user device. Pre-building is strongly recommended for the development loop — Berlin graph preparation takes several minutes, and rebuilding on every container restart is disruptive. The pre-built graph is a Docker volume snapshot or a committed artifact.
...
### GraphHopper Reachability and the Phase 1 Deployment Gap (acknowledge throughout phase 1)

In phase 1 the apps cannot route without network access to the GraphHopper instance. On the developer's own Mac this is invisible — localhost always works — but on iOS devices it means the device must reach the developer machine or the VPS over the network for any routing to happen. Demoing the app away from a known network, or using it on a real ride, requires either tethering to a hotspot on the same network as the GraphHopper host or hosting GraphHopper publicly on the VPS.

This is not a bug to fix in phase 1; it is the defining limitation of phase 1. The plan must not drift toward trying to make GraphHopper work offline on iOS — it cannot — and the team must not be surprised when an end-to-end demo requires network planning. The fix is phase 2: the embedded custom engine.

Resolution: host GraphHopper on the Hetzner VPS alongside the tile server as soon as iOS testing on real devices begins. Treat phase 1 explicitly as developer-facing, not user-facing.
